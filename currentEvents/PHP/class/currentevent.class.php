...
class CurrentEvent
{
    ...
    public function createFile()
    {
        ...
        $folder = PATH_CONTENT . 'current-events/';
        ...
        if (isset($_GET['edit'])) {
            ...
            echo ("<meta http-equiv='refresh' content='0;url=" . DOMAIN_ADMIN . "plugin/currentevents?addevent&edit=" . $_GET['edit'] . "'>");
        } else {
            global $SITEURL;
            global $GSADMIN;
            file_put_contents($folder . $hashedTitle . '.json', json_encode($event));
            echo ("<meta http-equiv='refresh' content='0;url=" . DOMAIN_ADMIN . "plugin/currentevents?addevent&edit=" . $hashedTitle . "'>");
        }
    }
};
